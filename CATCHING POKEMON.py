...
class PokemonGame:
    ...
    def menu_screen(self):
        ...
        self.canvas = tk.Canvas(self.root, width=1280, height=670)
        self.canvas.pack()
        ...
        self.buttons = [
            self.create_button("Tutorial", self.tutorial_screen, 640, 300),
            self.create_button("Play Game", self.start_game, 640, 400),
            self.create_button("Exit Game", self.root.quit, 640, 500),
        ]
        self.current_menu_index = 0
    ...
    def play_gif_background(self, canvas):
        gif_path = "C:\\Users\\Admin\\Desktop\\CATCHING POKEMON GAME\\TAGBG.gif"
    ...
    def create_button(self, text, command, x, y):
        button = tk.Button(self.root, text=text, font=("Black Ops One", 24), bg="white", command=command)
    ...
    def tutorial_screen(self):
        ...
        self.canvas = tk.Canvas(self.root, width=1280, height=670, bg="lightgreen")
        self.canvas.pack()
        self.canvas.create_text(640, 100, text="Tutorial", font=("Black Ops One", 60), fill="black")

        instructions = (
            "1. Nắm 4 ngón tay lại ngoài ngón cái để chọn nút.\n"
            "2. Chạm ngón trỏ và ngón cái để chuyển xuống nút tiếp theo.\n"
            "3. Chơi và tận hưởng trò chơi!\n"
        )
    ...
    def initialize_game(self):
        ...
        self.canvas = tk.Canvas(self.root, width=1280, height=640)
        self.canvas.pack()

        self.load_images()

        self.score = 0
        self.score_label = tk.Label(self.root, text=f"SCORE: {self.score}", font=("Bytebounce", 36))

        self.time_left = 30
        self.timer_label = tk.Label(self.root, text=f"TIME LEFT: {self.time_left}s", font=("Bytebounce", 36))
        ...
        self.score_label.pack(side="left", padx=280)  # Small gap to the right of the score label
        self.timer_label.pack(side="left", padx=160)  # Small gap to the left of the time label
    ...
    def end_game(self):
        ...
        end_bg_path = r"C:\\Users\\Admin\\Desktop\\CATCHING POKEMON GAME\\TAGBG.jpg"
        ...
        # Game Over text
        self.canvas.create_text(640, 250, text="GAME OVER", font=("Black Ops One", 80), fill="red")
        self.canvas.create_text(640, 350, text=f"FINAL SCORE: {self.score}", font=("Black Ops One", 40), fill="black")

        # Cursor reset
        self.root.config(cursor="arrow")

        # Main Menu button
        main_menu_button = self.create_button("MAIN MENU", self.menu_screen, 640, 500)

        self.buttons = [main_menu_button]
        self.current_menu_index = 0
        self.highlight_button(self.buttons[self.current_menu_index])

        # Start tracking sound for the "MAIN MENU" and "PLAY AGAIN" buttons
        if not self.sound_tracking_active:
            self.sound_tracking_active = True
    ...
    def setup_game_data(self):
        ...
        self.spawn_locations = {
            "Flying": [(240, 80), (320, 100), (400, 130)],
            "Water": [(580, 380), (600, 400), (640, 420)],
            "Land": [(300, 580), (350, 600), (400, 620)]
        }
